...
# pip uninstall Slackclient 한뒤 pip install Slackclient==1.0.2
from flask import Flask, request, make_response, render_template
# pip uninstall flask 한뒤 pip install flask==0.12.2
import secretKey

# secretKey.py 폴더안에 slack_token, slack_verification 모두 token값 설정
...
def _get_user_profile(userId):
    ...
    keywords = []
    data = {}
    data['nickname'] = soup.find('span', class_='p-nickname vcard-username d-block')
    ...
    keywords.append("\tnickName : " + data['nickname'])
...
def _get_boj(tag, level):
    ...
    if tag == 'dp':
        ...
        tag = '문자열 처리'
    url = "https://www.acmicpc.net/problem/tags/" + urllib.parse.quote(tag)
...
# 이벤트 핸들하는 함수
# 이 부분이 오류가 발생하는데 원인을 알려드릴 테니 한번 생각해보시면 좋을 듯?
...
def _event_handler(event_type, slack_event):
    if event_type == "app_mention":
        ...
        try:
            ...
            elif len(text) > 2 and match_text[1] is not None:
                keywords = _get_dd_contribution(text[1], match_text[1])
            else:
                keywords = ERR_TEXT
            ...
            STATUS_CODE = 500
            keywords = ERR_TEXT
            print("오류발생", e)  # 이 코드를 통해 오류가 무슨 오류인지 확인가능
        sc.api_call(
            "chat.postMessage",
            channel=channel,
            text=keywords
